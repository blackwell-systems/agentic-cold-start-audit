...
- You may run `--help` commands to discover metadata
- You may NOT execute actual test commands — that's the audit agent's job
- Write exactly one file: the filled prompt at `{{OUTPUT_PATH}}`
- The filled prompt must have zero placeholders remaining
- Every command in the audit areas must use the exec prefix
